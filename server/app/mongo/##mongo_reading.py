...
from mongo.mongo_users import update_user_fields
...
mongo_reading = Blueprint('mongo_reading', __name__)
...
reading_database = client.reading
...
# Get the demo module
@mongo_reading.route("/get_demo_module", methods=["GET"])
def get_demo_module():
    try:
        # Get the demo module from the Mongo DB
        demo_module = reading_database.reading_demo_module.find_one({"module.title": "Demo Module"})
    ...
    # Take care of any exceptions that may come up with working with the DB
        ...
        print(f"Error in getting reading demo module: {e}")
        return jsonify({"success": False, "message": f"Error getting reading demo module: {str(e)}"}), 500


# Get a random uncompleted reading module and if all are completed reset the array
@mongo_reading.route("/get_uncompleted_module", methods=["POST"])
def get_uncompleted_module():
    try:
        ...
        # Find out how many reading modules user has left
        # Need to convert all ObjectIds to strings in order to compare them
        completed_modules = set(str(x) for x in doc.get("reading", {}).get("modules_completed", []))
        modules = list(reading_database.reading_modules.find())
        uncompleted_modules = [a for a in modules if str(a["_id"]) not in completed_modules]

        # If user has no reading modules left then reset the array and return the response
        if not uncompleted_modules:
            # Reset the modules_completed array to be able to start working on reading modules from the start
            users_collection.update_one(
                {"user.email": user_email},
                {"$set": {"reading.modules_completed": []}}
            )
            return jsonify({
                "success": False, 
                "message": "You completed all reading modules so in order to continue your progress was reset."
                }), 200

        # Select a random remaining reading module and return it
    ...
    # Take care of any exceptions that may come up with working with the DB
        ...
        print(f"Error in getting an uncompleted reading module: {e}")
        return jsonify({"success": False, "message": f"Error getting uncompleted reading module: {str(e)}"}), 500


# Update intro_completed from False to True
@mongo_reading.route("/updates", methods=["PATCH"])
def updates():
    try:
        ...
        if "intro_completed" in data: 
            set_updates["reading.intro_completed"] = data["intro_completed"]

        # If module_completed is set in the request body then save it to the push_updates array
        push_updates = {}
        if "modules_completed" in data:
            push_updates["reading.modules_completed"] = data["modules_completed"]
    ...
    # Take care of any exceptions that may come up with working with the DB
        ...
        return jsonify({"success": False, "message": f"Error updating user reading: {str(e)}"}), 500